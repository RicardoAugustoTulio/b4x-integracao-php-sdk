...
class PedidosController extends BaseController
{
     ...
     */
    public function cancelaUmPedido(): void
    {
        $_reqBuilder = $this->requestBuilder(RequestMethod::DELETE, '/produtos/z1');
    ...
    /**
     * ###Listar pedidos
     * Retorna a relação de pedidos, em ordem crescente por data, dentro do intervalo de data e demais
     * filtros informados.
     * O envio da data inicial e final para consulta é obrigatório e ainda é possível incluir mais filtros
     * na consulta.
     *
     * _Query parameters_
     *
     * | API | Descrição |
     * | --- | --- |
     * | GET /pedidos/?data_inicial={data_inicial}&data_final{data_final}&status={status,
     * status}&offset={Offest}&Limit={Limit} | Retorna todos os pedidos disponíveis para baixa |
     * | GET /pedidos/?={id} | Retorna informações de um pedido específico |
     * | `_#GET /pedidos/?data_inicial=2020-01-01 00:00:01&data_final=2020-01-01 00:00:
     * 01&offset={Offest}&limit={Limit}&ingorar_integrados=1` |  |
     *
     * _Parâmetros da Url_
     *
     ...
     * | pagamentos | lista de pagamentos para o pedido | lista de \* [pagamentos](#api-pagamentos-do-
     * pedido) |
     ...
     * | notas_fiscais | lista de notas cadastradas no pedido | lista de \* [notas](#api-notas-fiscais-do-
     * pedido) |
     *
     * _Output_
     *
     ...
     *
     * **status=2 | descricao_status:confirmado**: indica que pedido já teve o pagamento confirmado e que
     * está pronto para a separação.
     *
     * **status=3 | descricao_status:separacao**: indica que os itens do pedido já estão em processo de
     * separação. A atualização para esse status é feito pelo fornecedor.
     *
     * **status=4 | descricao_status:faturado**: indica que a nota fiscal do pedido já foi emitida. A
     * atualização para esse status ocorre automaticante logo após o envio dos dados da NF pelo fornecedor.
     *
     * **status=5 | descricao_status:disponvel**: indica que pedido esta pronto para a coleta pela
     * transportadora ou disponível para o cleinte retirar no _local escolhido_
     *
     * **status=6 | descricao_status:transporte**: indica que o forncedor já entregou o pedido a
     * transportadora. A atualização para esse status é feito pelo fornecedor.
     *
     * **status=7 | descricao_status:entregue**: Indica que o pedido que já foi entregue ao cliente final.
     *
     * **status=8 | descricao_status:cancelado**: Indica que o pedidos foi cancelado. A atualização para
     * esse status pode ser feito pelo o lojista ou pelo ou pelo fornecedor.
     *
     * **status=9-problemas-entrega**: É o status que o pedido recebe quando por algum motivo a entrega não
     * foi realizada.
     *
     * @return void Response from the API call
     *
    ...
    {
        $_reqBuilder = $this->requestBuilder(RequestMethod::GET, '/pedidos');
     ...
     * @throws ApiException Thrown if API call fails
     */
    public function listarUmPedido(): void
    {
        $_reqBuilder = $this->requestBuilder(RequestMethod::GET, '/pedidos/393929');
     ...
     * @param string $codigo
     *
     * @return Pedido[] Response from the API call
     *
     * @throws ApiException Thrown if API call fails
     */
    public function pedido(string $codigo): array
    {
        $_reqBuilder = $this->requestBuilder(RequestMethod::GET, '/tickets/pedidos/{codigo}')
            ->parameters(TemplateParam::init('codigo', $codigo));

        $_resHandler = $this->responseHandler()->type(Pedido::class, 1);
     ...
     * @param string $cpf
     *
     * @return NewRequest6[] Response from the API call
     *
     * @throws ApiException Thrown if API call fails
     */
    public function pedidosCpf(string $cpf): array
    {
        $_reqBuilder = $this->requestBuilder(RequestMethod::GET, '/tickets/pedidosCpf/{cpf}')
            ->auth('basic')
            ->parameters(TemplateParam::init('cpf', $cpf));

        $_resHandler = $this->responseHandler()->type(NewRequest6::class, 1);
